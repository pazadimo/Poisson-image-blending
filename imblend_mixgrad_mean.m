function  output = imblend_mixgrad_mean( source, mask, target, transparent )
...
bred = bred(:);
bred2 = bred2(:);
bred2(indexes) = 4*bred(indexes)-bred(indexes_up)-bred(indexes_down)-bred(indexes_left)-bred(indexes_right);
...
% fuckred = cat(2, bred, bred2);
fuckred = (bred*transparent+ bred2*(1-transparent)) ;
% [~ ,argred]= max(abs(fuckred),[],2);
% bred(argred(indexes)==2) = bred2((argred(indexes)==2));
bred = fuckred;
...
bgreen = bgreen(:);
bgreen2 = bgreen2(:);
bgreen2(indexes) = 4*bgreen(indexes)-bgreen(indexes_up)-bgreen(indexes_down)-bgreen(indexes_left)-bgreen(indexes_right);
...
fuckgreen = cat(2, bgreen, bgreen2);
fuckgreen = (bgreen*transparent+ bgreen2*(1-transparent));
% [~ ,arggreen]= max(abs(fuckgreen),[],2);
% bblue(arggreen(indexes)==2) = bgreen2((arggreen(indexes)==2));
bgreen = fuckgreen;
...
bblue = bblue(:);
bblue2 = bblue2(:);
bblue2(indexes) = 4*bblue(indexes)-bblue(indexes_up)-bblue(indexes_down)-bblue(indexes_left)-bblue(indexes_right);
...
fuckblue = cat(2, bblue, bblue2);
fuckblue = (bblue*transparent+ bblue2*(1-transparent));

% [~ ,argblue]= max(abs(fuckblue),[],2);
% bblue(argblue(indexes)==2) = bblue2((argblue(indexes)==2));
bblue = fuckblue;
